...
import { postEvent, putEvent } from '@/common/utils/client/events';
import { UserInformation } from '@/common/utils/server/auth';
import { Event } from '@/common/models/event';
import { formatDate, formatDateInput } from '@/common/utils/client/format';
...
export interface EditEventModuleProps {
  userInfo: UserInformation,
  event: Event,
};

export const EditEventModule : React.FC<EditEventModuleProps> = ({ userInfo, event }) => {
  const router = useRouter();
  
  const [imageUrl, setImageUrl] = useState(event.image_url);
  const [startTime, setStartTime] = useState<Date>(new Date(event.start_time));
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<Inputs>({
    defaultValues: {
      ...
      startTime: formatDateInput(new Date(event.start_time)),
      endTime: formatDateInput(new Date(event.end_time)),
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    const success = await putEvent(event.id, {
      name: data.name,
